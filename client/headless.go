...
//
// uaOverride (optional) pins the navigator.userAgent VK's embedded JS sees,
// so the browser identity matches whatever the surrounding flow advertises.
func launchHeadlessCaptcha(url string, uaOverride string) func() {
	bin := findChromium()
	if bin == "" {
	...
	if uaOverride != "" {
		...
		opts = append(opts, chromedp.UserAgent(uaOverride))
	}
	_ = filepath.Base
	_ = strings.Contains
	if tmp != "" {
	...
	go func() {
		...
		if uaOverride != "" {
			actions = append(actions, emulation.SetUserAgentOverride(uaOverride))
		}
	...
	log.Printf("[Captcha] headless: chromedp launched %s (UA-override=%v)", bin, uaOverride != "")
...
// headlessUserAgent optionally overrides navigator.userAgent in the headless
// browser. Empty means Chromium's own UA. Set from -headless-ua.
var headlessUserAgent string

// manualCaptchaTimeout bounds how long the client waits for a captcha to be
// solved — by a human in manual mode, or by Chromium in headless mode.
// Default 5 minutes; see the -manual-captcha-timeout flag.
var manualCaptchaTimeout = 5 * time.Minute
